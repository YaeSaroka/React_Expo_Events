...
export default function Formulario({ route }) {
  ...
  const locations = async () => { 
    try {
      const response = await axios.get('http://10.152.2.141:3000/api/event-location', config);
  ...
  const categorias = async () => { 
    try {
      const response = await axios.get('http://10.152.2.141:3000/api/event-category', config);
  ...
  const handleCreateEvent = async () => { 
    try {
      const response = await axios.post('http://10.152.2.141:3000/api/event', {
        name,
        description,
        id_event_category,
        id_event_location,
